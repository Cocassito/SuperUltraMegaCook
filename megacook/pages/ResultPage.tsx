...
import GaugeSummary from "../components/ui/gaugeSummary/GaugeSummary";
...
export default function ResultPage({ selectedChefs, selectedModel, selectedBase, selectedAutre, onBack }: ResultPageProps) {
  ...
  const avg = {
    ...
    acidity: allNutritional.reduce((sum, item) => sum + item.acidity, 0) / count,
    protein: allNutritional.reduce((sum, item) => sum + item.protein, 0) / count,
    ...
    <View style={styles.background}>
      <View style={styles.receiptCard}>
        ...
        <View style={styles.gauges}>
          ...
          <Text style={styles.gaugeText}>Acidité : {avg.acidity.toFixed(2)}/5</Text>
          <Text style={styles.gaugeText}>Protéine : {avg.protein.toFixed(2)}/5</Text>
          <Text style={styles.gaugeText}>Gras : {avg.fat.toFixed(2)}/5</Text>
        </View>
        
        <Text style={styles.price}>Prix moyen : ${avg.price.toFixed(2)}</Text>
        
        <Text style={styles.selected}>Chef : {selectedChefs.map((key) => chefsData[key].name).join(', ')}</Text>
        
        <Text style={styles.backButton} onPress={onBack}>Suivant</Text>
      </View>
    </View>
  );
}
